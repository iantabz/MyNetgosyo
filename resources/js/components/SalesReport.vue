...
<template>
  <div id="page-body">
    <div class="panel">
              ...
              <div class="col-md-6">
                ...
                >
                  ...
                  <option
                    ...
                    :key="option.salesman_code"
                    :value="option.salesman_code"
                  >
                    {{
                      '[ ' +
      ...
      <!-- <div class="row mar-rgt mar-lft mar-btm">
            <div class="btn-group ">
              <button
                class="btn btn-default btn-active-purple"
                @click="printTopReport('product')"
                :disabled="salesman == ''"
              >
                Top Products
              </button>
      ...
      >
        ...
        <tbody>
          ...
          <tr v-for="(trans, index) in transactions.data" :key="index">
            <td>{{ trans.tran_no }}</td>
            <td style="font-size: 12px">{{ trans.store_name }}</td>
            <td>{{ trans.p_meth }}</td>
            <td>
              <span class="text-success">{{
                trans.date_req | formatDate
              }}</span>
            </td>
          ...
          <tr>
            <th colspan="5" style="text-align: right; font-size: 18px">
              Total
            </th>
            <th
              style="
...
export default {
    ...
    date(val, date2) {
      if (this.customer) {
        if (this.customer == 'All') {
          ...
          this.selectCustomer(salesMan.account_code)
        }
      }
    },
    date2(val) {
  ...
  methods: {
    ...
    printBtn2() {
      ...
      document.location.href = `/salesreport/printReport2/?data=${ids}&date=${btoa(
        this.date
    ...
    getResults(e) {
      ...
      const salesMan = this.customerList.filter(
        sm => sm.account_code == selectedSalesman
      )[0]
    ...
    selectCustomer(e) {
      axios
        .get(
          `/salesreport/customer/?code=${e}&date=${btoa(
            this.date
          )}&date2=${btoa(this.date2)}`
        )
        .then(response => {
